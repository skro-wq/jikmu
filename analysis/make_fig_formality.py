...
def presumption_holds(reason):
    """S2 추론 기록에서 '제6항 추정이 성립한다'는 판단을 집계한다(표 3의 마지막 열).

    정답 라벨이 아니라 에이전트가 남긴 문장을 규칙으로 분류한 값이다. 두 모델의
    표현 차이에 좌우되지 않도록 모델 무관 어구를 쓴다."""
    holds = reason.str.contains(
        "추정력이 있|추정이 있|추정.{0,3}작동|추정.{0,3}성립|법적 추정|간주.{0,4}충족|"
        "정당한 보상을 한 것으로|법적 최소요건.{0,4}충족|형식요건.{0,4}충족|"
        "요건을 (?:모두 )?갖춰|3요건.{0,4}충족", regex=True)
    fails = reason.str.contains(
        "추정.{0,4}(?:안|못|어렵|약|불충분|배제|깨|성립하지)|예외가 인정|추정.{0,4}받기 어렵",
        regex=True)
    return holds & ~fails
